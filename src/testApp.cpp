...
void testApp::StateMachineUpdate(void)
{
	...
	switch(CurrentState)
			...
			case DISPLAY_COUNTER_STATE:
				//cout << "DEBUG: Entered display counter state";
				elapsedTime = ofGetElapsedTimef() - DisplayCounterInitTime;
				if (((float)END_OF_THE_WORLD - elapsedTime)<0)
					ChangeToTakePhoto();
				break;
			case TAKE_PHOTO_STATE:
				//cout << "DEBUG: Entered take photo state";
				int status;
				
				status == ::PingCamera();
				
				status = ::ReleaseShutter(60, PhotoFilename, sizeof(PhotoFilename));
								/* check for DSLR remote status */
				switch(status)
					{
						case 1:
			...
			case ERROR_STATE:
				...
				break;
			default:
				cerr << "ERROR: unexpected system state: "  << endl;
			}
}
...
void testApp::ChangeToDisplayCounter(void)
{
	DisplayCounterInitTime = ofGetElapsedTimef();

	CurrentState = DISPLAY_COUNTER_STATE;
}

void testApp::ChangeToDisplayPhoto(void)
{
	DisplayPhotoInitTime = ofGetElapsedTimef();
	
	CurrentState = DISPLAY_PHOTO_STATE;
}

void testApp::ChangeToTakePhoto(void)
{		
